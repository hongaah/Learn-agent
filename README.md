...
每一课 README 的「跑一遍」都贴了真实的终端输出，但**你跑出来几乎不会和它逐字相同**。
模型每次的措辞、选择的命令、甚至完成任务用的轮数都可能不同——比如第 03 课的示例，
这次跑 3 轮、下次跑 4 轮，统计到的文件数也会随仓库内容变化。

这不是你配置错了，这是 agent 和普通程序最根本的区别：**同样的输入不保证同样的执行路径。**
实录的作用是让你知道"大致会发生什么"，以及在没有 API key 时也能完整理解这一课。

判断自己有没有跑对，看的是**关键现象**而不是逐字输出。每课第 4 节实录后面的点评
会指出这一课该看什么——比如 01 课要看第 2 次调用是否真的"不记得"，
03 课要看 `input_tokens` 是否逐轮变大，04 课要看 `safePath` 是否真的拦下了越界路径。

只有一种情况说明真的出问题了：**程序抛出未捕获的异常，或者返回 HTTP 400 加一段 HTML**。
后者见上面的「跑不通怎么办」。
